...
public final class MQSBuilder implements AutoCloseable, Closeable, CharSequence {
	...
	private final void lockedOffsetCAS(final booleanType from, final booleanType to) {
		while (!Constants.UNSAFE.compareAndSwapInt(this, lockedOffset, from.value, to.value)) {
		}
	}
	 ...
	 * 注意：长度必须 >0 并且 =Array.length<br/>
	 * 比append方法少一步 <code>if (len <= 0 || len > Array.length) return;</code><br/>
	 * <font color='red'>含有独占锁</font><br/>
	 * @param Array char[]
	 * @param len int
	 */
